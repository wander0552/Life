...
public class JsonConvert<T> implements Converter<T> {
     ...
     *     .tag(this)//
     *     .execute(new DialogCallback<CCResponse<ServerModel>>(this) {
     *          @Override
     *          public void onSuccess(CCResponse<ServerModel> responseData, Call call, Response response) {
    ...
    public T convertSuccess(Response response) throws Exception {
        ...
        //com.lzy.demo.callback.DialogCallback<com.lzy.demo.model.CCResponse<com.lzy.demo.model.ServerModel>> 得到类的泛型，包括了泛型参数
        Type genType = getClass().getGenericSuperclass();
        //从上述的类中取出真实的泛型参数，有些类可能有多个泛型，所以是数值
        Type[] params = ((ParameterizedType) genType).getActualTypeArguments();
        //我们的示例代码中，只有一个泛型，所以取出第一个，得到如下结果
        //com.lzy.demo.model.CCResponse<com.lzy.demo.model.ServerModel>
        Type type = params[0];
...
//        if (!(type instanceof ParameterizedType)) throw new IllegalStateException("没有填写泛型参数");
        ...
        //class com.lzy.demo.model.CCResponse
        ...
        if (typeArgument == Void.class) {
            //无数据类型,表示没有data数据的情况（以  new DialogCallback<CCResponse<Void>>(this)  以这种形式传递的泛型)
            ...
            simpleResponse.msg = response.body().string();
            response.close();
            return (T) simpleResponse.toLzyResponse();
        } else if (rawType == CCResponse.class) {
            //有数据类型，表示有data
            CCResponse lzyResponse = Convert.fromJson(jsonReader, type);
            response.close();
